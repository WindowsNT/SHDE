...
function PrepareDatabase()
{
    ...
    QQ("CREATE TABLE IF NOT EXISTS ORGANIZATIONS (ID INTEGER PRIMARY KEY,NAME TEXT,NAMEEN TEXT,LIMITCODES TEXT,SHDECODE TEXT,SHDECLIENT TEXT,SHDESECRET TEXT,SHDECLIENT2 TEXT,SHDESECRET2 TEXT,SHDEPRODUCTION INTEGER,DVGID INTEGER,DVGPASS TEXT)");
    QQ("CREATE TABLE IF NOT EXISTS ENDPOINTS (ID INTEGER PRIMARY KEY,OID INTEGER,PARENT INTEGER,NAME TEXT,NAMEEN TEXT,EMAIL TEXT,ALIASEMAIL TEXT,FORWARDEMAIL TEXT,LIMITCODES TEXT,T0 TEXT,T1 TEXT,T2 TEXT,T3 TEXT,T4 TEXT,T5 TEXT,T6 TEXT,T7 TEXT,T8 TEXT,T9 TEXT,A1 TEXT,A2 TEXT,A3 TEXT,TEL1 TEXT,TEL2 TEXT,TEL3 TEXT,SHDECODE TEXT,INTERNALCODE TEXT,INACTIVE INTEGER,DVGID INTEGER,DVGPASS TEXT,FOREIGN KEY (OID) REFERENCES ORGANIZATIONS(ID))");
    QQ("CREATE TABLE IF NOT EXISTS FOLDERS (ID INTEGER PRIMARY KEY,EID INTEGER,LID INTEGER,SPECIALID INTEGER,NAME TEXT,PARENT INTEGER,CLASSIFIED INTEGER,FOREIGN KEY (EID) REFERENCES ENDPOINTS(ID))");
    ...
    QQ("CREATE TABLE IF NOT EXISTS DOCUMENTS (ID INTEGER PRIMARY KEY,UID INTEGER,EID INTEGER,TOPIC TEXT,FID INTEGER,CLASSIFIED INTEGER,PRIORITY INTEGER,READSTATE INTEGER,PROT TEXT,TYPE INTEGER,CLSID TEXT,RECPX TEXT,RECPY TEXT,FROMX TEXT,FROMY TEXT,FROMZ TEXT,SHDERECEIPT TEXT,SHDEPROTOCOL TEXT,SHDEPROTOCOLDATE INTEGER,SHDEVERSION TEXT,SHDECHECKSENT TEXT,RECPZ TEXT,KOINX TEXT,KOINY TEXT,KOINZ TEXT,BCCX TEXT,BCCY TEXT,BCCZ TEXT,ESWX TEXT,COLOR TEXT,DUEDATE INTEGER,RELATED TEXT,DIAVGEIAID TEXT,KIMDISID TEXT,CATEGORY INTEGER,ORIGINALITY INTEGER,ORIGINALITYEXTRA TEXT,FORMATTING TEXT,ADDEDSIGNERS TEXT,METADATA TEXT,ENTRYCREATED INTEGER,PDFPASSWORD TEXT,SIGNERTITLES TEXT,EXPIRE INTEGER,QR INTEGER,FOREIGN KEY (FID) REFERENCES FOLDERS(ID),FOREIGN KEY (EID) REFERENCES ENDPOINTS(ID),FOREIGN KEY (UID) REFERENCES USERS(ID))");
    QQ("CREATE TABLE IF NOT EXISTS MESSAGES (ID INTEGER PRIMARY KEY,UID INTEGER,DID INTEGER,MSG TEXT,APPENDIX TEXT,DATE INTEGER,INFO TEXT,MIME TEXT,SIGNEDPDF BLOB,FOREIGN KEY (DID) REFERENCES DOCUMENTS(ID),FOREIGN KEY (UID) REFERENCES USERS(ID))");
    ...
    QQ("CREATE TABLE IF NOT EXISTS ORGCHART (ID INTEGER PRIMARY KEY,CODE TEXT,ROOTCODE TEXT,NAME TEXT,NAMEEN TEXT,ACTIVE INTEGER,SDDD INTEGER,PARENT INTEGER,FULLNAME TEXT,CODE2 TEXT,NEWENTRY INTEGER)");
    ...
    QQ("CREATE TABLE IF NOT EXISTS USERSINFOLDER (ID INTEGER PRIMARY KEY,UID INTEGER,FID INTEGER,ACCESS INTEGER,FOREIGN KEY (UID) REFERENCES USERS(ID),FOREIGN KEY (FID) REFERENCES FOLDERS(ID))");
    QQ("CREATE TABLE IF NOT EXISTS ORGCHARTCACHE (ID INTEGER PRIMARY KEY,MODE INTEGER,DATA BLOB)");
    QQ("CREATE TABLE IF NOT EXISTS DVG (ID INTEGER PRIMARY KEY,DID INTEGER,MID INTEGER,JSIN TEXT,JSOUT TEXT,FOREIGN KEY (DID) REFERENCES DOCUMENTS(ID),FOREIGN KEY (MID) REFERENCES MESSAGES(ID))");
}
...
function PrepareDatabaseMySQL()
{
    ...
    QQ("CREATE TABLE IF NOT EXISTS ORGANIZATIONS (ID INTEGER PRIMARY KEY AUTO_INCREMENT,NAME TEXT,NAMEEN TEXT,LIMITCODES TEXT,SHDECODE TEXT,SHDECLIENT TEXT,SHDESECRET TEXT,SHDECLIENT2 TEXT,SHDESECRET2 TEXT,SHDEPRODUCTION INTEGER,DVGID INTEGER,DVGPASS TEXT)");
    ...
    QQ("CREATE TABLE IF NOT EXISTS ORGCHART (ID INTEGER PRIMARY KEY AUTO_INCREMENT,CODE TEXT,ROOTCODE TEXT,NAME TEXT,ACTIVE INTEGER,SDDD INTEGER,PARENT INTEGER,FULLNAME TEXT,CODE2 TEXT,PARAG INTEGER,NEWENTRY INTEGER)");
    ...
    QQ("CREATE TABLE IF NOT EXISTS USERSINFOLDER (ID INTEGER PRIMARY KEY AUTO_INCREMENT,UID INTEGER,FID INTEGER,ACCESS INTEGER,FOREIGN KEY (UID) REFERENCES USERS(ID),FOREIGN KEY (FID) REFERENCES FOLDERS(ID))");
    QQ("CREATE TABLE IF NOT EXISTS ORGCHARTCACHE (ID INTEGER PRIMARY KEY AUTO_INCREMENT,MODE INTEGER,DATA BLOB)");
    QQ("CREATE TABLE IF NOT EXISTS DVG (ID INTEGER PRIMARY KEY AUTO_INCREMENT,DID INTEGER,MID INTEGER,JSIN TEXT,JSOUT TEXT,FOREIGN KEY (DID) REFERENCES DOCUMENTS(ID),FOREIGN KEY (MID) REFERENCES MESSAGES(ID))");
}
...
function CreateSpecialFoldersForEndpoint($eid)
{
    if (!QQ("SELECT * FROM FOLDERS WHERE EID = ?",array($eid))->fetchArray())
    {
        ...
        QQ("INSERT INTO FOLDERS (EID,SPECIALID,NAME) VALUES (?,?,?)",array($eid,FOLDER_LOCKERS,"Θυρίδες"));
    }
}
...
function LoadChart()
{
    $cacherow = QQ("SELECT * FROM ORGCHARTCACHE WHERE MODE = 1")->fetchArray();
    if ($cacherow)
        return  unserialize(GetBinary("ORGCHARTCACHE",'DATA',$cacherow['ID']));
    return null;
}
